...
ggplot(data = country_dis, aes(reorder(x = country, n), y = n, fill = country)) +
  ...
  coord_flip() +
  ggtitle("Barcode Data of Psittaciformes Distribution by Country") +
  xlab("Country") +
...
#Non-Metric Multidimensional Scaling (NMDS) is an ordination technique that can be used to reduce multivariate data to interpret patterns. Here NMDS is attempting to be used to visualize relationships between species and sites (countries). The NMDS technique arranges the points to maximize rank-order correlation. The commonly used Bray-Curtis community dissimilarity method is used.

#The metaMDS function is used to perform NMDS using the community object
NMDS <- metaMDS(bin.spread.by.country, distance = "bray", k =2)
plot(NMDS)
#The metaMDS function was unable to find a solution, likely due to insufficient data. Similarly, the plot is difficult to interpret

#The k value (number of reduced dimensions) can be increased as well as the number of iterations to see if a solution is reached
NMDS1 <- metaMDS(bin.spread.by.country, distance = "bray", k =3, trymax = 50)

#No solution is reached indicating it is likely due to insufficient data

#Check amount of NAs in country and BIN ID
sum(is.na(dfpsitt$country))
#1388 NAs
sum(is.na(dfpsitt$bin_uri))
#859 NAs

#The large amount of NAs in country and BIN ID could be a reason why the NMDS cannot be performed on the data
...
#Finally, in attempt to visualize the similarity of species by country, non-metric multidimensional scaling was implemented, however due to insufficient data, a solution was not reached. This further highlights the importance of sampling size when working with data. This dataset was not large enough to accurately explore species richness by country, however it is a beneficial tool in a larger analysis of monitoring how species richness changes over time as a result of human interference. In the future, it can be used in a longitudinal study of monitoring changes to species richness in countries. This would ultimately require near equal sampling of countries over time or a method of standardization. In conclusion, this analysis shows insight into the importance of sampling size when analysing data and provides insight on how this tool can be used on a larger scale for species diversity analysis.   
